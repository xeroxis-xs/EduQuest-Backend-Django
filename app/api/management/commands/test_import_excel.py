...
class Command(BaseCommand):
    ...
    def read_excel_sheets(self):
        ...
        excel_file = 'app/api/management/commands/NKKSNB_Tutorial_4_-_SCS3_Tue-PM-results.xlsx'
            ...
            while not pd.isnull(self.main_results_sheet.iloc[i, 0]):
                ...
                if self.main_results_sheet.iloc[i, 4].upper() == email.upper():
                    ...
                    for j in mcq_indices:
                        ...
                        elif wooclap_selected_answer_string[:4] == 'V - ' or wooclap_selected_answer_string[:4] == 'X - ':
        ...
        with open('app/api/management/commands/questions.json', 'w') as f:
            try:
                if user_question_attempt_list:
                    ...
                    print("Getting Question Attempts: Data successfully written to questions.json")
                else:
                    print("Getting Question Attempts: user_question_attempt_list is empty, nothing to write.")
            except Exception as e:
                print(f"Getting Question Attempts: Error writing to file: {str(e)}")
